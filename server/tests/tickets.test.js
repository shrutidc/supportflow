/**
 * Characterization + security tests for the ticket API.
 * Runs against an in-memory MongoDB with a stubbed session; no network.
 */
process.env.NODE_ENV = 'test';

const { test, before, after, beforeEach } = require('node:test');
const assert = require('node:assert/strict');
const mongoose = require('mongoose');
const { MongoMemoryServer } = require('mongodb-memory-server');
const request = require('supertest');

const createApp = require('../src/app');
const seedDB = require('../src/db/seed');
const Ticket = require('../models/Ticket');
const { fakeAuthMiddleware, as } = require('./helpers/auth');

const ORG = 'org_test';
const AGENT = as({ userId: 'user_agent', orgId: ORG, role: 'org:member' });
const MANAGER = as({ userId: 'user_manager', orgId: ORG, role: 'org:manager' });

let mongod;
let app;

before(async () => {
    mongod = await MongoMemoryServer.create();
    await mongoose.connect(mongod.getUri());
    app = createApp({ authMiddleware: fakeAuthMiddleware });
});

after(async () => {
    await mongoose.disconnect();
    await mongod.stop();
});

beforeEach(async () => {
    await seedDB(ORG);
});

// --- authentication ---

test('rejects unauthenticated requests', async () => {
    const res = await request(app).get('/api/tickets');
    assert.equal(res.status, 401);
});

test('rejects a signed-in user with no active organization', async () => {
    const res = await request(app).get('/api/tickets').set({ 'x-test-user': 'user_x' });
    assert.equal(res.status, 403);
});

// --- GET /api/tickets ---

test('lists all 20 seeded tickets with the list projection', async () => {
    const res = await request(app).get('/api/tickets').set(AGENT);
    assert.equal(res.status, 200);
    assert.equal(res.body.tickets.length, 20);

    const t = res.body.tickets[0];
    assert.ok(t.ticketId);
    assert.ok(t.customer.name);
    assert.ok(t.subject);
    // full documents are not leaked in list view
    assert.equal(t.messages, undefined);
});

test('filters by status', async () => {
    const res = await request(app).get('/api/tickets?status=Escalated').set(AGENT);
    assert.ok(res.body.tickets.length > 0);
    assert.ok(res.body.tickets.every(t => t.status === 'Escalated'));
});

test('searches subject with q, case-insensitively', async () => {
    const res = await request(app).get('/api/tickets?q=api').set(AGENT);
    assert.ok(res.body.tickets.length > 0);
    assert.ok(res.body.tickets.every(t => /api/i.test(t.subject)));
});

test('regex metacharacters in q are treated literally', async () => {
    const res = await request(app)
        .get('/api/tickets?q=' + encodeURIComponent('((['))
        .set(AGENT);
    assert.equal(res.status, 200);
    assert.deepEqual(res.body.tickets, []);
});

test('view=escalations returns only escalated tickets', async () => {
    const res = await request(app).get('/api/tickets?view=escalations').set(AGENT);
    assert.ok(res.body.tickets.length > 0);
    assert.ok(res.body.tickets.every(t => t.status === 'Escalated'));
});

test('view=assigned returns only tickets owned by the caller', async () => {
    // Seed data assigns by display name only, so nothing is owned by a user id yet.
    const before = await request(app).get('/api/tickets?view=assigned').set(AGENT);
    assert.equal(before.body.tickets.length, 0);

    await request(app).post('/api/tickets/SF-1001/claim').set(AGENT);

    const mine = await request(app).get('/api/tickets?view=assigned').set(AGENT);
    assert.equal(mine.body.tickets.length, 1);
    assert.equal(mine.body.tickets[0].ticketId, 'SF-1001');

    // Another agent's "assigned to me" must not include it.
    const theirs = await request(app).get('/api/tickets?view=assigned').set(MANAGER);
    assert.equal(theirs.body.tickets.length, 0);
});

test('sorts by lastUpdated descending by default, ascending on request', async () => {
    const desc = (await request(app).get('/api/tickets').set(AGENT)).body.tickets;
    for (let i = 1; i < desc.length; i++) {
        assert.ok(new Date(desc[i - 1].lastUpdated) >= new Date(desc[i].lastUpdated));
    }

    const asc = (await request(app).get('/api/tickets?sort=lastUpdated_asc').set(AGENT)).body
        .tickets;
    for (let i = 1; i < asc.length; i++) {
        assert.ok(new Date(asc[i - 1].lastUpdated) <= new Date(asc[i].lastUpdated));
    }
});

test('respects limit and falls back to default on garbage', async () => {
    const limited = await request(app).get('/api/tickets?limit=5').set(AGENT);
    assert.equal(limited.body.tickets.length, 5);

    const garbage = await request(app).get('/api/tickets?limit=banana').set(AGENT);
    assert.equal(garbage.status, 200);
    assert.equal(garbage.body.tickets.length, 20);
});

// --- GET /api/tickets/:ticketId ---

test('returns a full ticket by id, 404 when missing', async () => {
    const found = await request(app).get('/api/tickets/SF-1001').set(AGENT);
    assert.equal(found.status, 200);
    assert.equal(found.body.ticketId, 'SF-1001');
    assert.ok(Array.isArray(found.body.messages));

    const missing = await request(app).get('/api/tickets/SF-9999').set(AGENT);
    assert.equal(missing.status, 404);
    assert.deepEqual(missing.body, { error: 'Ticket not found' });
});

// --- POST /api/tickets/:ticketId/claim ---

test('claim assigns the ticket to the caller', async () => {
    const res = await request(app)
        .post('/api/tickets/SF-1001/claim')
        .set(as({ userId: 'user_agent', orgId: ORG, name: 'Alex Kim' }));

    assert.equal(res.status, 200);
    assert.equal(res.body.status, 'In Progress');
    assert.equal(res.body.assignedTo, 'Alex Kim');
    assert.equal(res.body.assignedToUserId, 'user_agent');
});

test('claiming an owned ticket conflicts with 409', async () => {
    await request(app).post('/api/tickets/SF-1001/claim').set(AGENT);

    const second = await request(app).post('/api/tickets/SF-1001/claim').set(MANAGER);
    assert.equal(second.status, 409);
    assert.deepEqual(second.body, { error: 'Ticket is already assigned' });
});

test('concurrent claims: exactly one wins (atomicity)', async () => {
    const claim = user =>
        request(app)
            .post('/api/tickets/SF-1001/claim')
            .set(as({ userId: user, orgId: ORG }));

    const results = await Promise.all([claim('u1'), claim('u2'), claim('u3')]);
    const statuses = results.map(r => r.status).sort();
    assert.deepEqual(statuses, [200, 409, 409]);

    const ticket = await Ticket.findOne({ organizationId: ORG, ticketId: 'SF-1001' });
    assert.ok(['u1', 'u2', 'u3'].includes(ticket.assignedToUserId));
});

test('claiming a missing ticket returns 404', async () => {
    const res = await request(app).post('/api/tickets/SF-9999/claim').set(AGENT);
    assert.equal(res.status, 404);
});

// --- PATCH /api/tickets/:ticketId ---

test('updates status and bumps lastUpdated', async () => {
    const beforeDoc = await Ticket.findOne({ organizationId: ORG, ticketId: 'SF-1001' });
    const res = await request(app).patch('/api/tickets/SF-1001').set(AGENT).send({ status: 'Closed' });

    assert.equal(res.status, 200);
    assert.equal(res.body.status, 'Closed');
    assert.ok(new Date(res.body.lastUpdated) > new Date(beforeDoc.lastUpdated));
});

test('escalation applies domain side effects and releases user ownership', async () => {
    await request(app).post('/api/tickets/SF-1001/claim').set(AGENT);

    const res = await request(app)
        .patch('/api/tickets/SF-1001')
        .set(AGENT)
        .send({ status: 'Escalated' });

    assert.equal(res.status, 200);
    assert.equal(res.body.priority, 'High');
    assert.equal(res.body.assignedTo, 'Engineering Queue');
    assert.equal(res.body.assignedToUserId, null);
    assert.ok(new Date(res.body.slaDeadline) > new Date());
});

test('resetting to New clears the assignee', async () => {
    await request(app).post('/api/tickets/SF-1001/claim').set(AGENT);
    const res = await request(app).patch('/api/tickets/SF-1001').set(AGENT).send({ status: 'New' });

    assert.equal(res.body.assignedTo, null);
    assert.equal(res.body.assignedToUserId, null);
});

test('agents cannot reassign tickets; managers can', async () => {
    const denied = await request(app)
        .patch('/api/tickets/SF-1001')
        .set(AGENT)
        .send({ assignedTo: 'Someone Else' });
    assert.equal(denied.status, 403);

    const allowed = await request(app)
        .patch('/api/tickets/SF-1001')
        .set(MANAGER)
        .send({ assignedTo: 'Someone Else' });
    assert.equal(allowed.status, 200);
    assert.equal(allowed.body.assignedTo, 'Someone Else');
});

test('mass-assignment fields are stripped from PATCH', async () => {
    const res = await request(app)
        .patch('/api/tickets/SF-1002')
        .set(AGENT)
        .send({
            status: 'In Progress',
            ticketId: 'SF-HACKED',
            organizationId: 'org_attacker',
            createdAt: '1999-01-01T00:00:00Z',
            customer: { name: 'Mallory' }
        });

    assert.equal(res.status, 200);
    assert.equal(res.body.ticketId, 'SF-1002');
    assert.equal(res.body.organizationId, ORG);
    assert.notEqual(new Date(res.body.createdAt).getFullYear(), 1999);
    assert.notEqual(res.body.customer.name, 'Mallory');
});

test('invalid enum values and empty patches are rejected with 400', async () => {
    const badStatus = await request(app)
        .patch('/api/tickets/SF-1001')
        .set(AGENT)
        .send({ status: 'Bogus' });
    assert.equal(badStatus.status, 400);

    const noFields = await request(app)
        .patch('/api/tickets/SF-1001')
        .set(AGENT)
        .send({ nonsense: true });
    assert.equal(noFields.status, 400);
});

// --- POST /api/tickets/:ticketId/messages ---

test('appends a message and bumps lastUpdated', async () => {
    const res = await request(app)
        .post('/api/tickets/SF-1002/messages')
        .set(AGENT)
        .send({ sender: 'agent', body: 'Engineering has a fix rolling out.' });

    assert.equal(res.status, 200);
    const last = res.body.messages[res.body.messages.length - 1];
    assert.equal(last.body, 'Engineering has a fix rolling out.');
});

test('first agent reply on a New ticket auto-claims it for the caller', async () => {
    const res = await request(app)
        .post('/api/tickets/SF-1001/messages')
        .set(as({ userId: 'user_agent', orgId: ORG, name: 'Alex Kim' }))
        .send({ sender: 'agent', body: 'Looking into this now.' });

    assert.equal(res.status, 200);
    assert.equal(res.body.status, 'In Progress');
    assert.equal(res.body.assignedTo, 'Alex Kim');
    assert.equal(res.body.assignedToUserId, 'user_agent');
});

test('client-supplied customer sender is rejected (fabrication guard)', async () => {
    const res = await request(app)
        .post('/api/tickets/SF-1001/messages')
        .set(AGENT)
        .send({ sender: 'customer', body: 'I demand a refund.' });
    assert.equal(res.status, 400);
});

test('rejects blank and missing message bodies with 400', async () => {
    const blank = await request(app)
        .post('/api/tickets/SF-1001/messages')
        .set(AGENT)
        .send({ sender: 'agent', body: '   ' });
    assert.equal(blank.status, 400);

    const missing = await request(app)
        .post('/api/tickets/SF-1001/messages')
        .set(AGENT)
        .send({ sender: 'agent' });
    assert.equal(missing.status, 400);
});

test('messages on a missing ticket return 404', async () => {
    const res = await request(app)
        .post('/api/tickets/SF-9999/messages')
        .set(AGENT)
        .send({ sender: 'agent', body: 'hello?' });
    assert.equal(res.status, 404);
});

// --- misc ---

test('unknown API routes return JSON 404', async () => {
    const res = await request(app).get('/api/does-not-exist').set(AGENT);
    assert.equal(res.status, 404);
    assert.deepEqual(res.body, { error: 'Not found' });
});

test('health check needs no credentials', async () => {
    const res = await request(app).get('/healthz');
    assert.equal(res.status, 200);
    assert.deepEqual(res.body, { status: 'ok' });
});

test('responses carry security and tracing headers', async () => {
    const res = await request(app).get('/api/tickets').set(AGENT);
    assert.equal(res.headers['x-content-type-options'], 'nosniff');
    assert.ok(res.headers['x-request-id']);
    assert.match(res.headers['content-security-policy'], /script-src 'self'/);
});
